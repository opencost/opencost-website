...
> **Note:** For Microsoft Customer Agreement (MCA) accounts, users will see an error when attempting to download the default price sheet. On-demand node pricing for MCA accounts is instead provided through the Azure Retail Prices API. If this limitation affects you, please open or comment on an issue so we can evaluate adding support in a future release.
