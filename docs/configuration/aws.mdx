...
OpenCost may be installed on Kubernetes clusters running on EC2 or on Elastic Kubernetes Service (EKS). You will need to install Prometheus, configure your cluster, and then install OpenCost.
...
<!-- The location should but automatically detected, but this URL can be overwritten using the environment variable `AWS_PRICING_URL`. -->
