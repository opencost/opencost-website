...
* `<BILLING_DATA_DATASET>` requires the full BigQuery table name in the format `dataset.table_name`. For example: `billing_dataset.gcp_billing_export_resource_v1_018AIF_74KD1D_534A2`.

Set these values into the GCP array in the `cloud-integration.json`:
...
``` json
{
  "gcp": [
    {
      "projectID": "<GCP_PROJECT_ID>",
      "billingDataDataset": "billing_dataset.gcp_billing_export_resource_v1_018AIF_74KD1D_534A2",
