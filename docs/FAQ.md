...
No. OpenCost is open source and free to use.

### How does OpenCost relate to Kubecost?
OpenCost is the cost allocation engine originally built by Kubecost. It’s actively used in the free Kubecost community version up to the Enterprise Kubecost version for cost allocation. However, Kubecost, along with a group of founding members, announced intentions to onboard OpenCost as a CNCF sandbox project. If the CNCF accepts our onboarding application, the CNCF will become the governing body for OpenCost moving forward.

### Why do I see Kubecost mentioned within OpenCost?
Kubecost originally developed OpenCost, so there are some legacy ties back to Kubecost. Moving forward, our goal is to fully separate OpenCost and Kubecost. 
